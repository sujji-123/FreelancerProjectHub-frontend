            ...
            <div className="max-w-4xl mx-auto">
                ...
                <div className="bg-white rounded-xl shadow-md p-6 mb-8">
                    <div className="flex flex-col sm:flex-row items-center gap-6">
                        {user.profilePicture ? (
                            <img src={`http://localhost:5001/${user.profilePicture}`} alt={user.name} className="w-24 h-24 rounded-full object-cover" />
                        ) : (
                         ...
                         {reviews.length > 0 ? reviews.map(review => (
                            <div key={review._id} className="bg-white rounded-xl shadow-md p-5 border border-gray-200">
                                <div className="flex items-start gap-4">
                                    {review.reviewer?.profilePicture ? (<img src={`http://localhost:5001/${review.reviewer.profilePicture}`} alt={review.reviewer.name} className="w-12 h-12 rounded-full object-cover" />) : (<FaUserCircle className="text-gray-400 text-5xl" />)}
